      ...
      if( isset($_POST['nnick']) )
      {
         ...
         else
         {
            ...
            if( $nu->set_password($_POST['npassword']) )
            {
               ...
               if( $nu->save() )
               {
                    if( isset($_POST['ncodperfil']) )
                    {
                       if($_POST['ncodperfil'] != '')
                       {
                            $perfil= new perfil();
                            $sperfil=$perfil->get_perfil($_POST['ncodperfil']);
                            foreach($sperfil->get_p_accesses() as $p)
                            {
                                $a = new fs_access( array('fs_user'=> $nu->nick, 'fs_page'=>$p->pa_page) );
                                $a->save();
                            }
                       }
                    }
